...
export default class ListAgents extends BaseTypedListCommand<ListAgentsResult> {
  ...
  protected getArtifactType(): 'agent' {
    return 'agent';
  }
  ...
  protected async scanLocal(projectPath: string): Promise<ScannedArtifact[]> {
    return LocalFileScanner.scanAgents(projectPath);
  }

  // eslint-disable-next-line class-methods-use-this
  protected buildResult(merged: MergedArtifact[]): ListAgentsResult {
