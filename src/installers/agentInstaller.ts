...
/**
 * Installer for agent artifacts.
 */
export class AgentInstaller extends Installer {
  public readonly artifactType: ArtifactType = 'agent';
  ...
  public async install(
    ...
    const installPath = PathResolver.resolve('agent', tool, artifact.name, projectPath);
    ...
    await Promise.all(
      artifact.files.map(async (file) => {
        ...
        const targetPath = PathResolver.resolveFile('agent', tool, artifact.name, targetName, projectPath);
  ...
  /**
   * Uninstall an agent from the project.
   */
  // eslint-disable-next-line class-methods-use-this
  public async uninstall(name: string, tool: string, projectPath: string): Promise<void> {
    const installPath = PathResolver.resolve('agent', tool, name, projectPath);
