...
/**
 * Installer for skill artifacts.
 */
export class SkillInstaller extends Installer {
  public readonly artifactType: ArtifactType = 'skill';
  ...
  public async install(
    ...
    const installPath = PathResolver.resolve('skill', tool, artifact.name, projectPath);
    ...
    await Promise.all(
      artifact.files.map(async (file) => {
        ...
        const targetPath = PathResolver.resolveFile('skill', tool, artifact.name, targetName, projectPath);
  ...
  public async uninstall(name: string, tool: string, projectPath: string): Promise<void> {
    const installPath = PathResolver.resolve('skill', tool, name, projectPath);
